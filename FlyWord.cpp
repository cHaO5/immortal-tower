#include "FlyWord.hpp"
/*FlyWord* FlyWord::create(const char *word,const int fontSize,auto begin){
...
bool FlyWord::init(const char *word,const int fontSize,auto begin){
    if(!Node::init()){
        return false;
    }
    //初始化
    _begin = begin;
    m_plabel = Label::create(word,Marker Felt,fontSize);
