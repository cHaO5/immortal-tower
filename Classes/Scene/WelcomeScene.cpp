...
void WelcomeScene::onEnterTransitionDidFinish()
{
	auto Sequence=Sequence::create(ScaleTo::create(0.5f,1.5f,1.5f),
		                           ...
		                           CallFuncN::create(CC_CALLBACK_0(WelcomeScene::init_continueBtn_startAnimation, this)),
		                           NULL);
...
void WelcomeScene::initBackGround()
{
	...
	auto sprite_background = Sprite::create("Scene/startbackground.png");
...
{
	//从SpriteFrameCache中加载图片
	sprite_Logo = Sprite::create("Scene/Logo.png");
	//设置锚点为图片中央
	sprite_Logo->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	//计算Logo图应该在的位置
	point_Logo.x = visibleSize.width / 2;
	point_Logo.y = visibleSize.height - (sprite_Logo->getContentSize().height / 2)+100;
	//设置位置,初始大小
...
{
	//从资源中加载图片
	button_Start = Sprite::create("Scene/play_on.png");
	//设置锚点为图片中央
	...
	button_Start_listener->onTouchBegan = [&](Touch* touch, Event* event) 
	{
		...
		if (rect.containsPoint(locationInNode) && target->isVisible())
		{
			...
			//改变Button式样达到点击效果
			target->setTexture("Scene/play_off.png");
			return true;
		}
	...
	{
		auto target = static_cast<Sprite*>(event->getCurrentTarget());
		static_cast<Sprite*>(event->getCurrentTarget())->setTexture("Scene/play_on.png");
...
void WelcomeScene::initButton_startAnimation()
{
	...
	//执行平移动画
	button_Start->runAction(MoveTo::create(0.3f, Point(point_Logo.x-200, point_Logo.y - 250)));//300
}


void WelcomeScene::init_continueBtn()
{
	button_credit = Sprite::create("Scene/crediton.png");
	button_credit->setVisible(false);
	//设置位置
	...
	button_credit_listener->onTouchBegan = [&](Touch* touch, Event* event)
	{
		...
		if (rect.containsPoint(locationInNode) && target->isVisible())
		{
			SoundManager::playClickEffect();
			//改变Button式样达到点击效果
			target->setTexture("Scene/creditoff.png");
			return true;
		}
		return false;
	};
	button_credit_listener->onTouchEnded = [&](Touch* touch, Event* event) {
		static_cast<Sprite*>(event->getCurrentTarget())->setTexture("Scene/crediton.png");
		Director::getInstance()->replaceScene(TransitionGame::create(1.0f, Information::createScene()));
	};
	_eventDispatcher->addEventListenerWithSceneGraphPriority(button_credit_listener, button_credit);
}

void WelcomeScene::init_continueBtn_startAnimation()
{
	//执行平移动画
	button_credit->runAction(Sequence::create(
		DelayTime::create(0.3f),
		CallFuncN::create(CC_CALLBACK_0(Sprite::setVisible, button_credit, true)),
		MoveTo::create(0.3f, Point(point_Logo.x+200, point_Logo.y - 250)), NULL));
}
...
void WelcomeScene::savemenuCloseCallback(cocos2d::Ref* pSender)
{
	...
	init_continueBtn_startAnimation();
}
