...
void BaseMonster::Move(Node *pSender)//此处加入ai算法
{
    ...
    if (GameManager::getInstance()->currentPlayer->getPosition().x - baseMonster->getPosition().x<0) baseMonster->setTexture("Monster0L.png");
    if (GameManager::getInstance()->currentPlayer->getPosition().x - baseMonster->getPosition().x>0) baseMonster->setTexture("Monster0R.png");
    if (GameManager::getInstance()->currentPlayer->getPosition().y - baseMonster->getPosition().y<-256) baseMonster->setTexture("Monster0D.png");
    if (GameManager::getInstance()->currentPlayer->getPosition().y - baseMonster->getPosition().y>256) baseMonster->setTexture("Monster0U.png");
