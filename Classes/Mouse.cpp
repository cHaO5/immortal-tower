...
bool Mouse::onTouchBegan(Touch *touch, Event *usused_event)
{
    ...
    //根据玩家武器类型创建不同的武器
    Sprite *projectile;
    if (_player->_weapon->_weapon == "weapon1") {
        projectile = Sprite::create("Projectile1.png");
    } else if (_player->_weapon->_weapon == "weapon2") {
        projectile = Sprite::create("Projectile2.png");
    }
    //auto projectile = Weapon::WeaponCreate(_player->_weapon);
    ...
    weaponcontrol->StartListen(_player->_weapon);
