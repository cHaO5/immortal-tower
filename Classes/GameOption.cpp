...
void ::GameOption::initButton()
{
	...
	music->setTag(1);
	if(UserDefault::getInstance()->getIntegerForKey("backmusic")==0)//0表示禁止BGM，1表示开启
	{
		addOff(music);
	}
	...
	effect->setTag(2);
	if(UserDefault::getInstance()->getIntegerForKey("backeffect")==0)//0表示禁止BGM，1表示开启
	{
		addOff(effect);	
	}
...
void GameOption::onTouchEnded(Touch* touch, Event* event)
{
	...
	case(1)://退出
	{
        //log("55555555555555555555555555555555");
        SimpleAudioEngine::getInstance()->pauseBackgroundMusic();
		Director::getInstance()->resume();
		onExitTransitionDidStart();
...
void GameOption::onExitTransitionDidStart()
{
